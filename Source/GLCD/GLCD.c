...
void DrawFilledCircle(int centerX, int centerY, int radius, uint16_t color) { 
    ...
    // Draw the initial vertical line filling the center of the circle
    LCD_DrawLine(centerX, centerY - radius, centerX, centerY + radius, color);
    ...
    while (x < y) {
        ...
        // Draw horizontal lines (scanlines) to fill the circle
        LCD_DrawLine(centerX - x, centerY - y, centerX + x, centerY - y, color);
        LCD_DrawLine(centerX - x, centerY + y, centerX + x, centerY + y, color);

        // Draw additional lines for symmetry
...
void DrawFilledSquare(int centerX, int centerY, int cellLength, uint16_t color) {
	int halfLength = cellLength / 2; // Half of the cell length to calculate bounds
	int x, y;
	for (x = centerX - halfLength; x <= centerX + halfLength; x++) {
		for (y = centerY - halfLength; y <= centerY + halfLength; y++) {
				LCD_SetPoint(x, y, color); // Set each pixel within the square to the specified color
		}
	}
}
...
void DrawPill(int x, int y, int halfCell,int radius){
	...
	for (dx = 0; dx < cellSize; dx++) {
		for (dy = 0; dy < cellSize; dy++) {
			...
			// Check if the point (dx, dy) lies within the circle
			if ((dx - halfCell) * (dx - halfCell) + (dy - halfCell) * (dy - halfCell) <= radius * radius) {
					LCD_SetPoint(x * cellSize + dy, y * cellSize + dx + margin_top, Yellow);
			}
		}
	}
}		
void DrawPacManRight(int centerX, int centerY, int radius, uint16_t color) {
    ...
    // Draw vertical lines at the center to start filling
    for (i = -radius; i <= radius; i++) {
        LCD_SetPoint(centerX, centerY + i, color);
    }
    ...
    while (x < y) {
        ...
        // Fill the circle, skipping pixels in the 45-degree "mouth" region
        for (i = -y; i <= y; i++) {
            if (!(x >= 0 && i >= 0 && x >= i) || x < 0) { // Skip the 45-degree slice (mouth)
                LCD_SetPoint(centerX + x, centerY + i, color);
            }
						    LCD_SetPoint(centerX - x, centerY + i, color);  // Left side (keep this intact)
        }
...
void DrawDoor(int centerX, int centerY, int size, uint16_t color){
	LCD_DrawLine(centerX - size/2, centerY - 1, centerX + size/2, centerY - 1, color);
	LCD_DrawLine(centerX - size/2, centerY , centerX + size/2, centerY, color);
	LCD_DrawLine(centerX - size/2, centerY + 1, centerX + size/2, centerY + 1, color);
}

void DrawGhost(int centerX, int centerY, int size, uint16_t color){
	LCD_DrawLine(centerX - 2 , centerY - 5, centerX + 1, centerY - 5, color);
	LCD_DrawLine(centerX - 3 , centerY - 4, centerX + 2, centerY - 4, color);
	LCD_DrawLine(centerX - 4 , centerY - 3, centerX + 3, centerY - 3, color);
	LCD_DrawLine(centerX - 5 , centerY - 2, centerX + 4, centerY - 2, color);
	LCD_DrawLine(centerX - 5 , centerY - 1, centerX + 4, centerY - 1, color);
	LCD_DrawLine(centerX - 5 , centerY, centerX + 4, centerY, color);
	LCD_DrawLine(centerX - 5 , centerY + 1, centerX + 4, centerY + 1, color);
	
	LCD_SetPoint(centerX - 5, centerY + 2, color);
	LCD_SetPoint(centerX - 4, centerY + 2, color);
	LCD_SetPoint(centerX - 1, centerY + 2, color);
	LCD_SetPoint(centerX, centerY + 2, color);
	LCD_SetPoint(centerX + 3, centerY + 2, color);
	LCD_SetPoint(centerX + 4, centerY + 2, color);

	LCD_SetPoint(centerX - 5, centerY + 3, color);
	LCD_SetPoint(centerX - 4, centerY + 3, color);
	LCD_SetPoint(centerX - 1, centerY + 3, color);
	LCD_SetPoint(centerX, centerY + 3, color);
	LCD_SetPoint(centerX + 3, centerY + 3, color);
	LCD_SetPoint(centerX + 4, centerY + 3, color);
	
	LCD_SetPoint(centerX - 5, centerY + 4, color);
	LCD_SetPoint(centerX - 4, centerY + 4, color);
	LCD_SetPoint(centerX - 1, centerY + 4, color);
	LCD_SetPoint(centerX, centerY + 4, color);
	LCD_SetPoint(centerX + 3, centerY + 4, color);
	LCD_SetPoint(centerX + 4, centerY + 4, color);
